package com.diagramsf.core.widget.pullrefresh.refreshview;
...
public class SunRefreshDrawable extends Drawable implements Animatable {
	...
	public SunRefreshDrawable(Context context, View parent,int refreshHeight) {
		mContext = context;
		mParent = parent;

		mMatrix = new Matrix();
		
		initiateDimens(refreshHeight);
		createBitmaps();
		setupAnimations();
	}
	...
	private void initiateDimens(int totalDragDistance) {
		...
		mTownFinalTopOffset = (mTotalDragDistance - mTownHeight
				* TOWN_FINAL_SCALE)
				+ mTotalDragDistance * .42f;
		mTownMoveOffset = AndroidUtil.convertDpToPixel(mParent.getContext(),10);
	...
	private void createBitmaps() {
		mSky = BitmapFactory.decodeResource(getContext().getResources(),
				R.drawable.sky);
		...
		mTown = Bitmap.createScaledBitmap(mTown, mScreenWidth,
				(int) (mScreenWidth * TOWN_RATIO), true);
		mSun = BitmapFactory.decodeResource(getContext().getResources(),
				R.drawable.sun);
	...
	private void drawSky(Canvas canvas) {
		...
		/**
		 * Change skyScale between {@link #SKY_INITIAL_SCALE} and 1.0f depending
		 * on {@link #mPercent}
		 */
		float skyScale;
		...
		/**
		 * less than {@link SCALE_START_PERCENT} will be
		 * {@link SKY_INITIAL_SCALE}
		 */
		...
		float offsetY = y + 50 + mSkyTopOffset // Offset canvas moving, goes
												// lower when goes down
				- mSkyHeight * (skyScale - 1.0f) / 2 // Offset sky scaling,
														// lower than 0, will go
														// greater when goes
														// down
				+ mSkyMoveOffset * dragPercent; // Give it a little move top ->
												// bottom // will go greater
												// when goes down
	...
	private void drawTown(Canvas canvas) {
		...
		if (scalePercentDelta > 0) {
			/**
			 * Change townScale between {@link #TOWN_INITIAL_SCALE} and
			 * {@link #TOWN_FINAL_SCALE} depending on {@link #mPercent} Change
			 * townTopOffset between {@link #mTownInitialTopOffset} and
			 * {@link #mTownFinalTopOffset} depending on {@link #mPercent}
			 */
			float scalePercent = scalePercentDelta
					/ (1.0f - SCALE_START_PERCENT);
			townScale = TOWN_INITIAL_SCALE
					+ (TOWN_FINAL_SCALE - TOWN_INITIAL_SCALE) * scalePercent;
			townTopOffset = mTownInitialTopOffset
					- (mTownFinalTopOffset - mTownInitialTopOffset)
					* scalePercent;
		...
		// float offsetY = (1.0f - dragPercent) * mTotalDragDistance // Offset
		// canvas moving
		float offsetY = y + +townTopOffset - mTownHeight * (townScale - 1.0f)
				/ 2 // Offset town scaling
	...
	private void drawSun(Canvas canvas) {
		...
		float offsetY = mSunTopOffset + (mTotalDragDistance / 2)
				* (1.0f - dragPercent); // Move the sun up
		...
		if (scalePercentDelta > 0) {
			...
			sunRotateGrowth += (SUN_FINAL_ROTATE_GROWTH - SUN_INITIAL_ROTATE_GROWTH)
					* scalePercent;
		...
		float r = (isRefreshing ? -360 : 360) * mRotate
				* (isRefreshing ? 1 : sunRotateGrowth);
