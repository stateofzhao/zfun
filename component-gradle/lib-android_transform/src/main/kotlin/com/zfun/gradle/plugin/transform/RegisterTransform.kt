...
import com.zfun.gradle.plugin.transform.utils.RegisterCodeGenerator
import com.zfun.gradle.plugin.transform.utils.TransformProcessUtil
...
class RegisterTransform : Transform() {
    ...
    companion object {
        ...
        val scanComponent:MutableList<ScanItem> = ArrayList()

        init {
            val scanProviderNames = AbsPluginLaunch.adapter.apiProviderClassSimpleNames()
            val scanProviderPackages = AbsPluginLaunch.adapter.apiProviderClassPackageNames()
            val size = scanProviderNames.size
            for (i in 0 until size){
                val aName = scanProviderNames[i]
                val aPackage = scanProviderPackages[i]
                scanComponent.add(ScanItem(aName, "$aPackage/$aName"))
            }
        }
    }
    ...
    override fun getName(): String {
        return AbsPluginLaunch.adapter.transformName()
    }
    ...
    override fun transform(transformInvocation: TransformInvocation) {
        ...
        scanComponent.forEach {
            it.apiProviderImplClassList.clear()
        }
    ...
    //=====jar
    private fun processJarInputWithIncremental(
        jarInput: JarInput,
        ...
        val desFile = outPutProvider.getContentLocation(
            destName + "_" + hexName,
    ...
    private fun transformDirectoryInput(directoryInput: DirectoryInput, desDir: File) {
        ...
        eachFileRecurse(directoryInput.file) {
            val desFile =
                File(it.absolutePath.replace(directoryInput.file.absolutePath, desDir.absolutePath))
