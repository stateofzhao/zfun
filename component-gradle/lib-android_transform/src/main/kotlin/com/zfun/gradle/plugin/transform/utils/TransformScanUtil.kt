...
object TransformProcessUtil {
    ...
    fun shouldProcessClassFile(path: String): Boolean {
        return path.startsWith(AbsPluginLaunch.adapter.annotationCompilerGenerateClassPackageName())
                || path.startsWith(AbsPluginLaunch.adapter.annotationCompilerGenerateClassPackageNameForWindows())
    }
    ...
    fun scanJar(srcFile: File, desFile: File) {
        ...
        if (srcFile.exists()) {
            ...
            while (enumeration.hasMoreElements()) {
                ...
                if (entryName.startsWith(AbsPluginLaunch.adapter.annotationCompilerGenerateClassPackageName())) {
                    //注解处理器生成的class
                    val inputStream = srcJarFile.getInputStream(jarEntity)
                    scanClass(inputStream)
                } else if (entryName == AbsPluginLaunch.adapter.injectTargetClassFileName()) {
                    Logger.i("scan jar##命中要注册的类所在jar：${srcFile.absolutePath}")
    ...
    override fun visit(version: Int, access: Int, name: String?, signature: String?, superName: String?, interfaces: Array<out String>?) {
        ...
        RegisterTransform.scanComponent.forEach { scanItem ->
            interfaces.forEach { aInterface ->
                ...
                if (aInterface == scanItem.apiProviderClassFullName) {
                    Logger.i("scan##命中注解解析器生成的类：${name}")
                    if (!scanItem.apiProviderImplClassList.contains(name)) {
                        scanItem.apiProviderImplClassList.add(name!!)
                    }
                }
            }
        }
