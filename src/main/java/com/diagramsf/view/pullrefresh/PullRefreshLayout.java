package com.diagramsf.view.pullrefresh;
...
@SuppressLint("ClickableViewAccessibility") public class PullRefreshLayout extends ViewGroup {
  ...
  private Animation mScrollAnimation = new Animation() {
    ...
    @Override protected void applyTransformation(float interpolatedTime, Transformation t) {
      ...
      setScrollToPosition(mAnimationScroll_isRefreshToNormal, mAnimationScroll_yDirection,
          (int) desX, (int) dexY);
      scrollingCallback();
    }
  };
  ...
  @TargetApi(Build.VERSION_CODES.HONEYCOMB) public void registerViewForScroll(View view) {
  ...
  public boolean canChildScrollUp(View view) {
    if (Build.VERSION.SDK_INT < 14) {
      ...
      return ViewCompat.canScrollVertically(view,
          -1);//这个有bug，就是当AbsListView有PaddingTop值时,并且设置了setClipToPadding(false),返回不正确
    }
  }
   ...
   */
  @TargetApi(Build.VERSION_CODES.HONEYCOMB) protected int getContentViewOffsetFromLeft() {
    if (mScrollMode == TranslationMode.SCROLL_TO) {
      return getScrollX();
   ...
   * @param y 目标y
   */
  @TargetApi(Build.VERSION_CODES.HONEYCOMB) protected void contentViewScrollToPosition(
      boolean isRefreshToNormal, float yDirection, int x, int y) {
    if (DEBUG) {
