...
/**
 * A request for retrieving a {@link NetResult} response body at a given URL,
 * allowing for an optional {@link NetResult} to be passed in as part of the
 * request body.
 */
public class NetResultRequest extends Request<NetResult> {
     ...
     */
    public NetResultRequest(int method, String url, String strRequest,
                            NetResultFactory resultFactory, Response.Listener<NetResult> listener,
     ...
     * @param errorListener 传递异常信息的接口
     */
    public NetResultRequest(int method, String url, Map<String, String> params,
                            NetResultFactory resultFactory, Response.Listener<NetResult> listener,
                            Response.ErrorListener errorListener) {
        super(method, url, errorListener);
        mResultFactory = resultFactory;
    ...
    @Override
    protected void deliverResponse(NetResult response) {
    ...
    protected Response<NetResult> parseNetworkResponse(NetworkResponse response) {
        try {
            ...
            NetResult.ResultType type = createAppResultType(response);

            if (null == mResultFactory) {
                return Response.success(null, null);
            }
            NetResult resultBean = mResultFactory.analysisResult(response.data,
                    response.headers);
