...
public abstract class RequestDataByVersion implements NetRequest {
    ...
    private NetResult mCacheLegitimacyResultData = null;// 缓存请求的合法结果
     ...
     */
    public RequestDataByVersion(String url, String postData, String cancelTag,
    ...
    NetRequestCallback mCallback = new NetRequestCallback() {
        @Override
        public void onSucceed(NetResult result) {
            NetResult.ResultType resultType = result.getResultType();
            boolean isLegitimacy = result.checkResultLegitimacy();
            if (resultType == NetResult.ResultType.CATCH) {// 来自缓存
                mRequestType = 2;
            ...
            } else if (resultType == NetResult.ResultType.NET) { // 来自网络
                if (mRequestType == 1) {// 证明本地没有缓冲第一次请求结果就来自网络
                    if (isLegitimacy) {
                        onSucceed(result);
                    } else {
                        if (mRecursionCounts >= Recursion_MAX) {
                            CommFailedResult failedResult = new CommFailedResult(new VolleyError("请求递归次数超过限制！"));
                            onFailed(failedResult);
                            return;
                        }
                        mRecursionCounts++;
                        if (StringUtils.isEmpty(mPostData)) {
                            mUrl = onUpdateLocalVersion();
                        } else {
                            mPostData = onUpdateLocalVersion();
                        }
                        mIsFirstRequest = false;
                        internalDoRequest(false);
                    }
                    ...
                    if (!isLegitimacy) {// 数据不合法,证明服务器端没有更新数据
                        if (null != mCacheLegitimacyResultData) {
                            // 将缓存的结果传递出去
                            onSucceed(mCacheLegitimacyResultData);
                        } else {
                            if (mRecursionCounts >= Recursion_MAX) {
                                CommFailedResult failedResult = new CommFailedResult(new VolleyError("请求递归次数超过限制！"));
                                onFailed(failedResult);
    ...
    private void internalDoRequest(boolean clear) {
        ...
        switch (mRequestType) {
            case 0:
                break;
            case 1:
                request = new NetRequestImpl(mUrl, mPostData, mFactory, mCancelTag);
                request.setDeliverToResultTag(mDeliverToResultTag);
                request.setCacheKey(mCacheKey);
                request.setCallBack(mCallback);
                request.doRequest(NetRequest.PRIORITY_CACHE);
                break;
            case 2:
                request = new NetRequestImpl(mUrl, mPostData, mFactory, mCancelTag);
                request.setDeliverToResultTag(mDeliverToResultTag);
    ...
    /** 只有 {@link NetResult#checkResultLegitimacy()} 合法，才会调用这个方法传递结果 */
    public abstract void onSucceed(NetResult response);
