    ...
    public void run() {
        ...
        while (true) {
            try {
                ...
                if (!request.isSkipCache()) {//分支都是我自己添加的
                    entry = mCache.get(request.getCacheKey());//这个是原始的
                }

                if (request.justReadCache()) {//分支都是我自己添加的
                    Response<?> response;
                    if (entry == null) {
                        request.addMarker("cache-miss");
                        response = Response.success(null,null);
                    } else {
                ...
                if (!request.readCacheWithoutTimeLimit()) {//我自己添加的
                ...
                if (!entry.refreshNeeded()
                    ...
                    // 获取到了过期的缓存.可以传递缓存结果，但是我们仍然需要发送网络请求来刷新结果。
