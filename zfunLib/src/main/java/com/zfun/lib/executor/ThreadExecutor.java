 ...
 *
 * Created by zfun on 2016/8/8 11:33
 */
public class ThreadExecutor implements Executor {
  ...
  protected ThreadExecutor() {
    ...
    ExecutorService service = new MyThreadPoolExecutor(CORE_SIZE, MAX_SIZE, TIMEOUT, TimeUnit.SECONDS, queue);
  ...
  @Override
  public boolean isRunningOrWaiting(Object tag) {
    return dispatcher.isContainer(tag);
  }
  ...
  private static class PriorityFuture extends FutureTask<Object> implements Comparable<PriorityFuture> {
    ...
    @Override protected void done() {
      try {
        ...
        //此处出现其它异常，一般是 run() 方法中出现了异常，需要抛出去，以便调用者能够发现自己传递过来的Runnable#run()方法写的bug。
      } catch (CancellationException ignore) {//如果被取消掉了，（会在调用cancel()方法的同一个线程中执行）
  ...
  private static class Dispatcher {
    ...
    private final DispatcherHandler handler;//主线程Handler
    private final Map<Object, List<PriorityFuture>> futureMap;
    ...
    boolean isContainer(Object tag){
      final List<?> tasks = futureMap.get(tag);
