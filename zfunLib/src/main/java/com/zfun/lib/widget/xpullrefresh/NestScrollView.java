...
/**
 * 来自sdk的源码，为了学习NestedScrolling用的。<br/>
 ...
 * <p/>
 * 嵌套滚动框架设计是这样的：<br/>
 * 父在嵌套滚动中完全不处理嵌套滚动相关touch事件，仅仅被动接收滚动事件；<br/>
 * 子在嵌套滚动中全权处理嵌套滚动相关touch事件。<br/>
 * <p/>
 * NestedScrollingParent 和 NestedScrollingChild 是链式调用，一般一个View（命名为A）既要实现 NestedScrollingParent
 * 也要实现 NestedScrollingChild；<br/>
 * NestedScrollingParent 相关方法会被 A 的子View调用；<br/>
 * NestedScrollingChild 相关方法则是 A 自己来调用；<br/>
 * 所以 A 作为 NestedScrollingParent 是与其子View交互的；<br/>
 * 作为 NestedScrollingChild 是与其父View交互的；<br/>
 * 这里容易搞蒙，会不自觉的认为是自己与自己在交互！
 * <p/>
 * 综上可以看到嵌套滚动时TOUCH事件归属为 NestedScrollChild 。
 * <p/>
 * Created by zfun on 2021/11/25 5:10 下午
 */
public class NestScrollView extends FrameLayout implements NestedScrollingParent2,NestedScrollingChild2, ScrollingView {
    ...
    public interface OnScrollChangeListener {
         ...
         */
        void onScrollChange(NestScrollView v, int scrollX, int scrollY,int oldScrollX, int oldScrollY);
    }
     ...
     */
    //注意 mScrollOffset 和 mScrollConsumed 意义是不一样的，一般情况下值是相等的，但是如果想要滚动视觉差，这俩就不一样了
    private final int[] mScrollOffset = new int[2];//parent自己真正滚动的值
    private final int[] mScrollConsumed = new int[2];//parent消耗的此次touch事件滚动的值
    ...
    public void onNestedScrollAccepted(@NonNull View child, @NonNull View target, int axes, int type) {
        ...
        //触发嵌套滚动时，继续向上传递嵌套滚动事件（将自己转换为 NestedScrollingChild 角色），
        // 这样就做到了无论是主动scroll还是被动scroll都做到了自己的 NestedScrollingChild 职责。
        //还有这里的参数为 ViewCompat.SCROLL_AXIS_VERTICAL，而不是直接用传递进来的 axes ，
        // 因为这里是自己作为 NestedScrollingChild 职责维护的，所以应当传递自己的特性。
    ...
    public boolean onNestedFling(View target, float velocityX, float velocityY, boolean consumed) {
        if (!consumed) {
            ...
            return true;//因为在 flingWithNestedDispatch() 方法中，无论自己是否消耗fling，都对fling进行了处理（通过Scroller分发了 onNestedScroll(ViewCompat.TYPE_NON_TOUCH)）
        }
        return false;
    }
    ...
    //这里容易理解错误，容易理解为其父View的值！
    //正确的理解是，自己作为 NestedScrollingParent 监听到的其子View的值，换句话说这个返回的是其子View的值！
    // 如果子View在此次Touch事件中将要处理滚动事件就返回 ViewCompat.SCROLL_AXIS_VERTICAL/ViewCompat.SCROLL_AXIS_HORIZONTAL；
    // 如果子View在此次Touch事件中不会处理滚动事件就返回 ViewCompat.ViewCompat.SCROLL_AXIS_NONE。
    //
    //这个值在子View调用 startNestedScroll(int axes, int type) 方法时被赋值。
    // 所以这个一般在 父View的onInterceptTouchEvent()#ACTION_MOVE 中判断。
    //
    //可以理解为在 NestedScrolling 框架没有引入时的，判断子View是否滚动到头了方法。【这句话是错误的！！】
    // 在NestedScrolling 框架中无所谓子View是否滚动到头了，因为作为父View时自己所有的滚动都是由子View触发的！！
    @Override
    ...
    //注意下变量的命名 yDiff(就是两个数的差值),yDelta(增量可以正可以负)，很科学，以后要模仿使用。
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_MOVE: {
                ...
                //(getNestedScrollAxes()&ViewCompat.SCROLL_AXIS_VERTICAL) == ViewCompat.SCROLL_AXIS_NONE
                // 这个判断很重要，要明白这个判断首先要明白 getNestedScrollAxes() 返回值的含义。
                // 这个判断的含义是如果其子View没有滚动，那么自己就要来处理自己的滚动了（如果子View有滚动那么不能拦截子View的事件，要让子View完成自己的滚动）
            ...
            case MotionEvent.ACTION_DOWN: {
                ...
                mVelocityTracker.addMovement(ev);
                //如果自己正在fling，那么需要接管（拦截）touch事件；
                //否则，不要在 ACTION_DOWN 时拦截touch事件，因为一旦拦截了就会阻止子View接收到 ACTION_DOWN 事件，造成子View永远无法拿到后续touch事件。
                //
                //如果当前正在fling，那么mScroller.isFinished()会返回false，mIsBeingDragged就为true。
                //首先调用一次 mScroller.computeScrollOffset() 方法，是为了确保 mScroller.isFinished() 此时返回真确的值。
                /*
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        //如果父View消耗了自身的滚动事件，会造成 MotionEvent 实际坐标变了，但是手指相对自身却没有变化，
        // 为了让 MotionEvent 实际坐标相对自身来说也没变，就需要让 MotionEvent 改变下坐标。
        ...
        switch (actionMasked) {
            case MotionEvent.ACTION_DOWN: {
                ...
                //如果正在 fling 那么一定要阻止父View的 touch 拦截，防止出现自己仍然在 fling 但父View却开始处理 touch 事件的错误逻辑
                ...
                mActivePointerId = ev.getPointerId(0);
                startNestedScroll(ViewCompat.SCROLL_AXIS_VERTICAL, ViewCompat.TYPE_TOUCH);
                //ACTION_DOWN 始终返回true，符合此ViewGroup的逻辑，因为"点中"此View就是想滚动，
                // 当然如果父View有啥特殊需求可以在onInterceptEvent()中拦截掉（就没有机会执行此View的onTouchEvent()方法了）
                break;
            }
            case MotionEvent.ACTION_MOVE:
                ...
                int deltaY = mLastMotionY - y;//注意这里是 上次的坐标 - 当前的坐标，所以如果手指向上滑动（子View向上滚动，子View露出下面的内容）为正
                //询问父View需要消耗多少滚动值
                if (dispatchNestedPreScroll(0, deltaY, mScrollConsumed, mScrollOffset, ViewCompat.TYPE_TOUCH)) {
                    deltaY -= mScrollConsumed[1];
                    vtev.offsetLocation(0, mScrollOffset[1]);//实时偏移掉parent消耗的滚动值
                    mNestedYOffset += mScrollOffset[1];//更新累计的被parent消耗掉的滚动值
                }
                //处理自己的 ACTION_MOVE 事件
                //
                ...
                if (mIsBeingDragged) {
                    ...
                    //在这里开始处理自己的滚动逻辑，这是一个容易忽略的地方，容易认为自己的滚动都是子View的touch事件来回调，
                    // 自己只需要处理好 NestedScrollingParent 接口就好了；这种理解也没有错，但是没有考虑到子View是非滚动View的情况，
                    // 所以这里仍然需要自己来处理下自己的滚动。
                    // Calling overScrollByCompat will call onOverScrolled, which
                    // calls onScrollChanged if applicable.
                    if (overScrollByCompat(0, deltaY, 0, getScrollY(), 0, range, 0,
                            0, true) && !hasNestedScrollingParent(ViewCompat.TYPE_TOUCH)) {
                        // 手动拖动到了边界，那么当手指离开时不应该触发fling了
            ...
            case MotionEvent.ACTION_UP:
                ...
                velocityTracker.computeCurrentVelocity(1000, mMaximumVelocity);//units参数的意思是：每秒处理多少个像素值，一般为1000
                //velocityY 带方向，正值为手指向下滑动，子View向下滚动（漏出上面的内容）；负值为手指向上滑动，子View向上滚动（漏出下面的内容）
                // 在证明 VelocityTracker 中计算两个touchEvent的距离也是 currentEvent.getY() - mLastEvent.getY()，
                // 只有本类中奇葩是 mLastEvent.getY() - currentEvent.getY()
                int initialVelocity = (int) velocityTracker.getYVelocity(mActivePointerId);
                if ((Math.abs(initialVelocity) > mMinimumVelocity)) {//纵轴速度需要处理成fling
                    flingWithNestedDispatch(-initialVelocity);
                } else if (mScroller.springBack(getScrollX(), getScrollY(), 0, 0, 0,getScrollRange())) {//判断滚动是否超出了边界
                    ViewCompat.postInvalidateOnAnimation(this);
                }
                mActivePointerId = INVALID_POINTER;
                endDrag();
    ...
    boolean overScrollByCompat(int deltaX, int deltaY,
        ...
        if (clampedY && !hasNestedScrollingParent(ViewCompat.TYPE_NON_TOUCH)) {
            //springBack()方法：判断给定的坐标 newScrollX newScrollY 是否处于 (minX,maxX,minY,maxY) 矩形范围内，
            // 返回true表示不在范围内，返回false表示在范围内。
            mScroller.springBack(newScrollX, newScrollY, 0, 0, 0, getScrollRange());
        }

        //滚动到制定位置，调用这个View的API可以自动触发 onScrollChanged() 监听
    ...
    public void fling(int velocityY) {
        if (getChildCount() > 0) {//有子View时，自己无论如何都会消耗掉fling
    ...
    private void endDrag() {
        ...
        recycleVelocityTracker();
        stopNestedScroll(ViewCompat.TYPE_TOUCH);//可以多次调用，并且不需要和startNestedScroll()成对调用
    ...
    static class AccessibilityDelegate extends AccessibilityDelegateCompat {
        ...
        public boolean performAccessibilityAction(View host, int action, Bundle arguments) {
            ...
            final NestScrollView nsvHost = (NestScrollView) host;
